...
export default function EVisitorsPage() {
  return (
    <PublicPage>
      ...
      <section className="bg-brand-secondary px-6 py-20 text-white sm:px-10 lg:px-16 lg:py-28"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:items-start"><SectionHeading dark eyebrow="The scanner workflow" title="From the first scan to a clear operational record." description="E-Visitors connects the moment someone arrives with the information your team needs to make a confident decision." /><div className="grid gap-3 sm:grid-cols-2">{workflow.map((item) => { const Icon = item.icon; return <div key={item.step} className="rounded-xl border border-white/20 bg-white/10 p-5"><div className="flex items-center justify-between"><span className="text-sm font-black text-white/60">{item.step}</span><Icon className="size-5 text-white" aria-hidden="true" /></div><h2 className="mt-8 text-2xl font-black tracking-[-0.04em]">{item.title}</h2><p className="mt-2 text-sm leading-6 text-white/70">{item.description}</p></div>; })}</div></div></section>
